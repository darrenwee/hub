...
func init() {
	...
	cmdListPulls.Flag.IntVarP(&flagIssueLimit, "limit", "L", -1, "LIMIT")
...
func listPulls(cmd *Command, args *Args) {
	...
	for _, pr := range pulls {
		if numWidth := len(strconv.Itoa(pr.Number)); numWidth > maxNumWidth {
...
func formatPullRequest(pr github.Issue, format string, colorize bool) string {
	base := pr.Base.Ref
	head := pr.Head.Label
	if pr.IsSameRepo() {
		head = pr.Head.Ref
	...
	placeholders := formatIssuePlaceholders(pr, colorize)
	placeholders["B"] = base
	placeholders["H"] = head
